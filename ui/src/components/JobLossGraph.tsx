...
export default function JobLossGraph({ job, embedded = false }: Props) {
  ...
  }, [series, activeKeys, colorByKey, smoothing, plotStride, windowSize, useLogScale, showTrend, clipOutliers, embedded]);
          ...
          <div className={controlPanelClassName}>
            <div className={embedded ? 'flex items-center gap-1.5' : ''}>
              ...
              <div className={embedded ? 'flex flex-wrap gap-1' : 'flex flex-wrap gap-2'}>
                <ToggleButton checked={showTrend} onClick={() => setShowTrend(v => !v)} label="Trend" compact={embedded} />
                ...
                <div className={embedded ? 'flex max-h-10 flex-1 flex-wrap gap-1 overflow-y-auto pr-1' : 'flex flex-wrap gap-2'}>
                  {lossKeys.map(k => (
                    <button
                      ...
                      title={k}
                    >
                      <span className="inline-block h-1.5 w-1.5 rounded-full mr-1" style={{ background: colorByKey[k] }} />
                      {k}
                    </button>
                  ))}
                </div>
